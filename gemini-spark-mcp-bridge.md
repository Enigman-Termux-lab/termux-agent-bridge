...
[Бесплатный постоянный домен Ngrok (*.ngrok-free.dev)]
         │
         │ Секретный путь (Secret Subpath): /spark-secret-token-placeholder/sse
...
6. **Почему Direct Capability URL лучше OAuth:**
   ...
   - Прямое подключение по длинному секретному адресу (`/spark-secret-token-placeholder/sse`) с `Authentication: None` работает безотказно: боты не могут угадать 128-битный путь, а Gemini видит все действия сразу.
...
4. Заполните поля:
   ...
   - **MCP Server URL:** `https://ВАШ-ДОМЕН.ngrok-free.dev/spark-secret-token-placeholder/sse`
