...
# Новый и старый секретные пути для надёжности
NEW_SECRET_PATH = "spark-693cff05cc2e0b79420ffe3d"
OLD_SECRET_PATH = "spark-sec-9a8f7b2c"
...
app = mcp.streamable_http_app(
    streamable_http_path=f"/{NEW_SECRET_PATH}/mcp",
    transport_security=security_settings,
)

endpoint = app.routes[0].endpoint

# Регистрируем алиасы для нового секретного пути
app.routes.append(Route(f"/{NEW_SECRET_PATH}", endpoint=endpoint))
app.routes.append(Route(f"/{NEW_SECRET_PATH}/sse", endpoint=endpoint))

# Регистрируем алиасы для старого пути (на случай если используется он)
...
if __name__ == "__main__":
    ...
    print(f"[+] Новый защищённый URL: /{NEW_SECRET_PATH}/sse")
    print(f"[+] Старый защищённый URL: /{OLD_SECRET_PATH}/sse")
