...
def load_config() -> dict[str, Any]:
    ...
    if not ngrok_domain:
        ngrok_domain = "unstaffed-clamshell-overplant.ngrok-free.dev"
...
def create_gateway_server(config: dict[str, Any]) -> FastMCP:
    ...
    # =========================================================================
        ...
        def bash_run(command: str, cwd: str | None = None) -> str:
            ...
            is_dang, reason = approval_daemon.is_dangerous_command(command)

            prefix_msg = ""
            if is_dang:
                print(f"[*] Запрос подтверждения опасной команды: {command} (Причина: {reason})")
                approved, verdict = approval_daemon.request_approval(
                    command=command,
                    reason=reason,
